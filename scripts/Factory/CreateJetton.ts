...
import { JettonFactory } from '../../wrappers/Factory';
...
export async function run(provider: NetworkProvider) {
    ...
    // Convert PoolCore_ADDRESS to Address type
const poolCoreAddress = Address.parse(PoolCore_ADDRESS);
    ...
    const sampleJetton = provider.open(await JettonCore.fromInit(provider.sender().address as Address, content,Max_Supply,initial_Price,iinitial_mint_Amount,poolCoreAddress)); //1000000000000000000n
